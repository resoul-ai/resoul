...
class XTTSBackend(TTSBackend):
    def __init__(self, model_name: str = "tts_models/multilingual/multi-dataset/xtts_v2", progress_bar: bool = False,
                 device: bool = True, **kwargs):
...
class CSMBackend(TTSBackend):
    ...
    def __call__(self, text: str, output_path: str, reference_audio_path: str = None, speaker: int = 0, context: list = None, max_audio_length_ms: int = 10_000, **kwargs) -> None:
        if context is None:
            context = []
...
@click.option("--reference-audio-path", default=DEFAULT_REFERENCE_SPEAKER,
...
@click.option("--ckpt-converter", type=click.Path(), default=CKPT_CONVERTER,
...
@click.argument("input-path", type=click.Path(exists=True))
@click.argument("output-dir", type=click.Path())
@click.option("--speaker", default=0, help="Speaker ID")
@click.option("--max-audio-length-ms", default=10_000, help="Maximum audio length in milliseconds")
@click.option("--device", default=DEVICE, help="Device for processing (cuda:0, cpu)")
def files(input_path: str, output_dir: str, speaker: int, max_audio_length_ms: int, device: str):
    """Generate speech using CSM model"""
    engine = CSMBackend(device=device)
    process_input(
        tts=engine,
        input_path=input_path,
        output_dir=output_dir,
...
@click.option("--max-audio-length-ms", default=10_000, help="Maximum audio length in milliseconds")
@click.option("--device", default=DEVICE, help="Device for processing (cuda:0, cpu)")
def query(text: str, output_path: str, speaker: int, max_audio_length_ms: int, device: str):
    """Generate speech using CSM model"""
    engine = CSMBackend(device=device)
    engine(
        text=text,
